<template>
    <div>
        <Address-Header title="修改收货地址"></Address-Header>
        <div class="address">
...
export default {
  data() {
	  return {
		  name : this.address[this.index].name,
		  phone : this.address[this.index].phone,
		  zone : this.address[this.index].zone,
		  detail : this.address[this.index].detail,
		  address : [],
	  }
  },
  components : {
  ...
  methods : {
	  save() {
		if (
			...
			this.$store.state.address[this.index] = data;
